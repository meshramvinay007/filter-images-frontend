...
function App() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setCategories(getNewCategories(categories));
        setDefaultCategories(getNewCategories(categories));
      })
  ...
  useEffect(() => {
    if (filters.length) {
      axios
        .post(`http://localhost:4000/getFilteredImages?page=${currentPage}`, {
          categories: filters,
        })
        .then((res) => {
          setLoading(false);
      ...
      axios
        .get(`http://localhost:4000/getAllImages?page=${currentPage}`)
        .then((res) => {
          setLoading(false);
          setImages(res.data.results);
          setTotalPages(res.data.totalPages);
